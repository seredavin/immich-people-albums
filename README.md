...
**ВАЖНО:** Перед запуском убедитесь, что файл `config.yaml` существует в той же директории, что и `docker-compose.yml`:

```bash
# Используйте скрипт проверки (рекомендуется):
./scripts/check-config.sh

# Или проверьте вручную:
# 1. Проверьте, что config.yaml - это файл, а не директория:
ls -la config.yaml
# Должно показать: -rw-r--r-- ... config.yaml (минус в начале означает файл)
# Если показывает: drwxr-xr-x ... config.yaml (d в начале означает директорию), то:
#   rmdir config.yaml  # если директория пустая
#   или rm -rf config.yaml  # если в ней что-то есть

# 2. Если файла нет, создайте его из примера:
cp config.example.yaml config.yaml
# Затем отредактируйте config.yaml и заполните свои данные
```

**Использование готового образа из registry (рекомендуется):**

1. Убедитесь, что `config.yaml` существует и заполнен
2. Убедитесь, что сеть настроена правильно
3. Запустите:
...
1. Убедитесь, что `config.yaml` существует и заполнен
2. Убедитесь, что `docker-compose.yml` настроен правильно
3. Обновите сеть в `docker-compose.yml`, если Immich использует другую сеть
4. Запустите:
